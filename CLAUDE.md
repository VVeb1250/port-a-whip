...
- [x] **router adapter** → `adapters/router.py`: `portaw router run` (hook entry, error→exit0 safe), `router test/enable/disable/status`. enable patches host settings.json UserPromptSubmit (backup+idempotent), CC+Gemini. tests/test_router.py (21). Verified live: secret→secure-agent, navigate→efficiency-starter, docs→context-quality, unrelated→silent.
...
- [x] **mistakes-index.md → lessons harvester** 2026-06-10 (`portaw/memory/harvest.py`, `memory harvest` CLI, 13 tests). The proven high-value capture path (the Bash detector yields ~0; the user's curated index is the gold source). Parses `- [SEV] [id] trigger → fix (xN, date) →detail` bullets → global lessons: severity→confidence (HIGH 0.9/MED 0.7/LOW 0.5), `(xN)`→recurrence, date→last_seen, section→applicability (`### Python`→stack:python, COMMANDS/SHELL→universal, WORKFLOW→project:curated), `→detail`→`detail_ref` (sibling mistakes-detail.md#id), backtick code-spans→trigger_terms+symbols. **Idempotent** (re-key by content-hash id, max-recurrence merge — re-harvest never inflates). Preview-by-default, `--confirm` writes. **LIVE-VALIDATED** on the real Thai-mixed index: 18 lessons parsed, applicability correct, run2 = 0 new/18 updated, py-command recurrence stays ×33. Full loop proven: harvest → `memory recall "python3 not found"` surfaces the py lesson.
- [x] **embedding tier-2** 2026-06-10 (`portaw/kernel/embed.py`, opt-in `[embed]` extra, 9 tests). Multilingual MiniLM (ONNX) ported from the author's skill-router `embed.py`, generalized skill-dict→Capability. **Lazy + optional + fail-safe**: fires ONLY when TF-IDF tier-1 returns nothing (heavy model never runs on a lexical hit); `available()` False → silent TF-IDF floor (runtime deps stay click+tomlkit). Reuses the author's already-downloaded model by default (`~/.claude/hooks/models`, or `$PAW_EMBED_MODEL_DIR`). Wired as an injectable `embed_fn` into `kernel.route()` + `memory.recall()` (default None → pure tier-1 → **parity 14/14 + all prior tests unchanged**); `make_embedder()` builds it, `memory recall --embed` exercises it. Corpus vectors cached by signature. **LIVE-PROVEN**: Thai prompt → English corpus cross-lingual match (`สแกนหาความลับ`→secure-agent); raw cosine 0.592 on a real semantic pair, {} sub-threshold (honest silence).
