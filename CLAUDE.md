...
- ⚠️ **L3 axis NON-NEGOTIABLE = cross-host portability + mistake-surfacing** (NOT general agent memory — jcode already does intra-host general memory better; paw redundant if it drifts off these 2 axes). deep-vet §7.
...
- **No sub-agent capability type**: multi-agent ≈ 15× tokens + loses on coding. If ever added: needs contract (objective/output-format/tool-guidance/boundaries) + breadth-first-only gate.
