...
- **DECISION (2026-06-13, owner): curated set auto-run ได้** — `portaw install <set> --run` (confirm-gated, `-y` ข้าม). exec = argv list ผ่าน `shell=False` (shlex.split → subprocess; Windows `.cmd`/`.bat` ห่อด้วย `cmd /c` + fixed argv) → **ไม่ตีความ shell metachar = ไม่มี RCE** แม้ step จะรัน vendor installer (npm/npx/winget). idempotent: tool ที่อยู่บน PATH แล้ว (healthcheck) ถูก skip. default (ไม่มี `--run`) ยัง PRINT. `--run` ปฏิเสธ set ที่ `untrusted:true` (community/Phase-4 — print-only จนกว่า hash/sig). narrows §12 ไม่ widen. → `portaw/sets/runner.py`.
...
- non-MCP install: **ห้าม shell=True เด็ดขาด** — RCE vector. curated set auto-run (`--run`) ได้ แต่เป็น argv ล้วน (`shell=False`, ไม่ตีความ metachar) — ดู §6 design rules. set `untrusted`/community = print-only จนกว่า hash/sig verify (Phase 4)
